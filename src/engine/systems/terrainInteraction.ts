...
import type { SystemState, ActiveFire } from '../SystemState'
...
export function terrainInteractionSystem(
  ...
  const activeFires = sys.activeFires
...
export function igniteForest(map: BattleMap, x: number, y: number, radius: number, fires: ActiveFire[]) {
  ...
  for (let dy = -cellRadius; dy <= cellRadius; dy++) {
    for (let dx = -cellRadius; dx <= cellRadius; dx++) {
      ...
      if (nr >= 0 && nr < map.terrain.length && nc >= 0 && nc < map.terrain[0].length) {
        if (map.terrain[nr][nc] === 'forest') {
          if (!fires.some((f) => f.col === nc && f.row === nr)) {
            fires.push({ col: nc, row: nr, age: 0, maxAge: 60 })
          }
        }
      }
    }
